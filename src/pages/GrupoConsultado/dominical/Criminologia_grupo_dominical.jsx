...
export default function CriminologiaDominical_grupo() {
  const [alumno, setAlumno] = useState([]);

  // 🔹 Endpoint específico para Criminología Dominical
  useEffect(() => {
    axios.get('http://localhost:4000/ConsultarAlumnosCriminologiaDominical')
  ...
  const exportPDF = () => {
    ...
    doc.text("Lista de Alumnos - Criminología Dominical", 14, 20);
    ...
    doc.save("alumnos_criminologia_dominical.pdf");
  ...
  const exportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Criminología Dominical");

    XLSX.writeFile(workbook, "alumnos_criminologia_dominical.xlsx");
  ...
  return (
    <main className="min-h-screen bg-gray-50 text-gray-800">
      ...
      <Navbar titulo="Licenciatura en Criminología (Dominical)" />
              ...
              {alumno.length === 0 ? (
                <tr>
                  <td colSpan="7" className="text-center py-6 text-gray-500 italic">
                    No hay alumnos registrados en Criminología Dominical.
                  </td>
                </tr>
              ) : (
                alumno.map((data, i) => (
