...
export default function SistemasDominical() {
  const [alumno, setAlumno] = useState([]);

  // 🔹 Endpoint adaptado a la modalidad dominical
  useEffect(() => {
    axios.get('http://localhost:4000/ConsultarAlumnosSistemasDominical')
  ...
  const exportPDF = () => {
    ...
    doc.text("Lista de Alumnos - Ingeniería en Sistemas (Modalidad Dominical)", 14, 20);
    ...
    doc.save("alumnos_sistemas_dominical.pdf");
  ...
  const exportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Sistemas Dominical");

    XLSX.writeFile(workbook, "alumnos_sistemas_dominical.xlsx");
  ...
  return (
    <main className="min-h-screen bg-gray-50 text-gray-800">
      {/* 🔹 Navbar con título */}
      <Navbar titulo="Ingeniería en Sistemas - Modalidad Dominical" />
              ...
              {alumno.length === 0 ? (
                <tr>
                  <td colSpan="7" className="text-center py-6 text-gray-500 italic">
                    No hay alumnos registrados en la modalidad dominical.
                  </td>
                </tr>
              ) : (
                alumno.map((data, i) => (
