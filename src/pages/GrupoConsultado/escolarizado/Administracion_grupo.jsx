...
export default function Administracion_grupo() {
  const [alumno, setAlumno] = useState([]);

  // 🔹 Endpoint adaptado a /ConsultarAlumnosAdministracion
  useEffect(() => {
    axios.get('http://localhost:4000/ConsultarAlumnosAdministracion')
  ...
  const exportPDF = () => {
    ...
    doc.text("Lista de Alumnos - Carrera de Administración", 14, 20);
    ...
    doc.save("alumnos_administracion.pdf");
  ...
  const exportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Alumnos Administración");

    XLSX.writeFile(workbook, "alumnos_administracion.xlsx");
  ...
  return (
    <main className="min-h-screen bg-gray-50 text-gray-800">
      <Navbar titulo="Carrera de Administración - Alumnos" />
