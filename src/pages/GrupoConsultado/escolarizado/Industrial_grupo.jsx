...
export default function Industrial_grupo() {
  const [alumno, setAlumno] = useState([]);

  // 🔹 Endpoint adaptado a Ingeniería Industrial
  useEffect(() => {
    axios.get('http://localhost:4000/ConsultarAlumnosIndustrial')
  ...
  const exportPDF = () => {
    ...
    doc.text("Lista de Alumnos - Ingeniería Industrial", 14, 20);
    ...
    doc.save("alumnos_ingenieria_industrial.pdf");
  ...
  const exportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Alumnos Ingeniería Industrial");

    XLSX.writeFile(workbook, "alumnos_ingenieria_industrial.xlsx");
  ...
  return (
    <main className="min-h-screen bg-gray-50 text-gray-800">
      {/* 🔹 Título del Navbar */}
      <Navbar titulo="Ingeniería Industrial - Alumnos" />
              ...
              {alumno.length === 0 ? (
                <tr>
                  <td colSpan="7" className="text-center py-6 text-gray-500 italic">
                    No hay alumnos registrados en Ingeniería Industrial.
                  </td>
                </tr>
              ) : (
                alumno.map((data, i) => (
