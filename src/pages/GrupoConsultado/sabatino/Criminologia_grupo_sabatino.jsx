...
export default function CriminologiaSabatino() {
  const [alumno, setAlumno] = useState([]);

  // 🔹 Llamada al endpoint de Criminología Sabatino
  useEffect(() => {
    axios.get('http://localhost:4000/ConsultarAlumnosCriminologiaSabatino')
  ...
  const exportPDF = () => {
    ...
    doc.text("Lista de Alumnos - Criminología Sabatino", 14, 20);
    ...
    doc.save("alumnos_criminologia_sabatino.pdf");
  ...
  const exportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Alumnos Criminología Sabatino");

    XLSX.writeFile(workbook, "alumnos_criminologia_sabatino.xlsx");
  ...
  return (
    <main className="min-h-screen bg-gray-50 text-gray-800">
      {/* 🔹 Navbar con el título de la carrera */}
      <Navbar titulo="Criminología Sabatino - Alumnos" />
              ...
              {alumno.length === 0 ? (
                <tr>
                  <td colSpan="7" className="text-center py-6 text-gray-500 italic">
                    No hay alumnos registrados en Criminología Sabatino.
                  </td>
                </tr>
              ) : (
                alumno.map((data, i) => (
                  ...
                  >
                    ...
                    <td className="py-3 px-4">{data.semestre}</td>
                  </tr>
                ))
              )}
            </tbody>
