...
export default function PsicologiaSabatino_grupo() {
  const [alumno, setAlumno] = useState([]);

  // 🔹 Endpoint adaptado a Psicología Sabatino
  useEffect(() => {
    axios.get('http://localhost:4000/ConsultarAlumnosPsicologiaSabatino')
  ...
  const exportPDF = () => {
    ...
    doc.text("Lista de Alumnos - Psicología (Sabatino)", 14, 20);
    ...
    doc.save("alumnos_psicologia_sabatino.pdf");
  ...
  const exportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Alumnos Psicología Sabatino");

    XLSX.writeFile(workbook, "alumnos_psicologia_sabatino.xlsx");
  ...
  return (
    <main className="min-h-screen bg-gray-50 text-gray-800">
      <Navbar titulo="Licenciatura en Psicología - Modalidad Sabatina" />
              ...
              {alumno.length === 0 ? (
                <tr>
                  <td colSpan="7" className="text-center py-6 text-gray-500 italic">
                    No hay alumnos registrados en Psicología Sabatino.
                  </td>
                </tr>
              ) : (
                alumno.map((data, i) => (
