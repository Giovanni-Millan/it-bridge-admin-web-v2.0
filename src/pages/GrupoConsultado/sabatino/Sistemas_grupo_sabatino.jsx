...
export default function Semestre6SistemasSabatino() {
  const [alumno, setAlumno] = useState([]);

  // 🔹 Ruta actualizada al endpoint correcto
  useEffect(() => {
    axios.get('http://localhost:4000/ConsultarAlumnosSistemasSabatino')
  ...
  const exportPDF = () => {
    ...
    doc.text("Ingeniería en Sistemas - Modalidad Sabatina (Semestre 6)", 14, 20);
    ...
    doc.save("alumnos_sistemas_sabatino_semestre6.pdf");
  ...
  const exportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Sistemas Sabatino - Semestre 6");

    XLSX.writeFile(workbook, "alumnos_sistemas_sabatino_semestre6.xlsx");
  ...
  return (
    <main className="min-h-screen bg-gray-50 text-gray-800">
      {/* 🔹 Título del navbar */}
      <Navbar titulo="Ingeniería en Sistemas - Sabatino (Semestre 6)" />
              ...
              {alumno.length === 0 ? (
                <tr>
                  <td colSpan="7" className="text-center py-6 text-gray-500 italic">
                    No hay alumnos registrados en este semestre sabatino.
                  </td>
                </tr>
              ) : (
                alumno.map((data, i) => (
