...
export default function CarrerasPorSeleccion() {
  ...
  const obtenerCarreras = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from("carrera")
        .select("*") // Traemos todos los campos necesarios (nombre completo para extraer modalidad)
        .order("id_carrera");
  ...
  // Función para agrupar carreras por modalidad (extraída del componente original)
  const agruparPorModalidad = () => {
    ...
    carreras.forEach((carrera) => {
      ...
      const modalidad = partes[1].toLowerCase().trim();
  ...
  const irAlumnosCarrera = (id_carrera, nombre) => {
    if (!id_carrera) return;
    navigate(`/alumnos/carrera/${id_carrera}`, {
      state: { nombreCarrera: nombre },
    });
  };

  // Definición de secciones con título, ícono representativo y colores
  const secciones = [
    {
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
                    {carrerasSeccion.map((carrera) => (
                      <button
                        ...
                        onClick={() => irAlumnosCarrera(carrera.id_carrera, carrera.nombre)}
